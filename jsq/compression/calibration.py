...
from jsq.compression.utils import get_named_linears
...
@torch.no_grad()
def get_layer_act_scale(module, inps, **layer_kwargs):
    ...
    def cache_input_hook(m, x, y, name):
        x = x[0]
        stat_tensor(name, x)
    ...
    inps = inps.to(next(module.parameters()).device)
    
    module(inps, **layer_kwargs)[0]
    for h in handles:
        h.remove()
...
@torch.no_grad()
def get_block_input_feat(module, inps, **layer_kwargs):
    named_linears = get_named_linears(module)
    ...
    inps = inps.to(next(module.parameters().device)) # in case multi-gpu
    inps = module(inps, **layer_kwargs)[0]
    return 
